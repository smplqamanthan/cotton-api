...
app.get("/api/cotton-mixing-summary-progress", async (req, res) => {
  try {
    ...
    const { from_date, to_date, unit, line, cotton, mixing, mixing_from, mixing_to } = req.query;
    ...
    let issueQuery = supabase.from("mixing_issue").select("unit, line, mixing_no, cotton, issue_date").limit(50000);
    if (parsedFromDate) issueQuery = issueQuery.gte("issue_date", parsedFromDate);
    if (parsedToDate) issueQuery = issueQuery.lte("issue_date", parsedToDate);
    ...
    if (cottonFilters.length > 0) {
      issueData = issueData.filter((row) => {
        if (!row || row.cotton === undefined || row.cotton === null) return false;
        const cottonFilterSet = new Set(cottonFilters.map((v) => String(v).trim().toUpperCase()));
    ...
    const totalMixing = validMixNos.length;
    
    res.write(`data: ${JSON.stringify({ type: "total", count: totalMixing })}\n\n`);
    res.end();
  } catch (error) {
...
// 🚀 Weighted average summary endpoint with streaming progress (filter by issue_date)
app.get("/api/cotton-mixing-summary", async (req, res) => {
  try {
    ...
    const enableSummaryDebug = true; // process.env.SUMMARY_DEBUG === "true";
    ...
    const sendFinalData = (data) => {
      res.write(`data: ${JSON.stringify({ type: "data", data })}\n\n`);
      res.end();
    };

    if (!["daily", "weekly", "monthly"].includes(report_type)) {
      const error = new Error("Invalid report_type. Must be 'daily', 'weekly', or 'monthly'.");
      throw error;
    }
...
if (!issueData || issueData.length === 0) {
      return sendFinalData([]); // No issues in the given date range
    }
    ...
    issueData.forEach((issueRow) => {
      const unitKey = issueRow.unit ?? "";
      const lineKey = issueRow.line ?? "";
      const cottonKey = issueRow.cotton ?? "";
      const mixingKey = issueRow.mixing_no ?? "";
    ...
    // 3️⃣ Fetch corresponding mixing_chart entries (with chunking to avoid Supabase limits)
    let mixingData = [];
    if (validMixNos.length > 0) {
      const chunkSize = 25;
        for (let i = 0; i < validMixNos.length; i += chunkSize) {
        const chunk = validMixNos.slice(i, i + chunkSize);
        let mixingQuery = supabase.from("mixing_chart").select("*").limit(50000);
        mixingQuery = mixingQuery.in("mixing_no", chunk);
        ...
        const { data: chunkData, error: chunkError } = await mixingQuery;
        if (chunkError) throw chunkError;
        if (chunkData) {
          mixingData = mixingData.concat(chunkData);
        }
      }
    } else {
      // If no validMixNos, still apply other filters
      let mixingQuery = supabase.from("mixing_chart").select("*").limit(50000);
      if (unitFilters.length > 0) mixingQuery = mixingQuery.in("unit", unitFilters);
      if (lineFilters.length > 0) mixingQuery = mixingQuery.in("line", lineFilters);
      if (effectiveCottonFilters.length > 0) {
      ...
      const { data: queryData, error: queryError } = await mixingQuery;
      if (queryError) throw queryError;
      mixingData = queryData || [];
    }

    if (!mixingData || mixingData.length === 0) {
       return sendFinalData([]); // No matching mixings found
    }
...
const { data: lotResults, error: lotError } = await supabase
  ...
  .limit(50000);
    ...
    if (report_type === "daily") {
      // Group by individual mixing entries (each mixing should be its own daily group)
      const groups = {};
      for (const issue of issueData) {
        const groupKey = `${issue.mixing_no ?? ""}|${issue.unit ?? ""}|${issue.line ?? ""}|${issue.cotton ?? ""}|${issue.issue_date}`;
        if (!groups[groupKey]) groups[groupKey] = { issue, rows: [] };
      }

      // Add mixing_chart rows to their corresponding groups
      let matchedRows = 0;
      let unmatchedRows = 0;
      for (const row of mixingData) {
        const lookupKey = `${row.unit ?? ""}|${row.line ?? ""}|${row.mixing_no ?? ""}|${row.cotton ?? ""}`;
        const issueDate = issueDateLookup[lookupKey] || "";
        const groupKey = `${row.mixing_no ?? ""}|${row.unit ?? ""}|${row.line ?? ""}|${row.cotton ?? ""}|${issueDate}`;
        if (groups[groupKey]) {
      ...
      // Compute weighted averages for each individual mixing entry
      for (const group of Object.values(groups)) {
        groupCounter++;
        const { issue, rows } = group;
        // For daily reports, include all mixing entries even if they have no mixing chart data
        ...
        // For daily reports, each mixing is its own entry
        const mixingNo = issue.mixing_no;
        const numericMixing = Number(mixingNo);
        const minMixing = Number.isNaN(numericMixing) ? null : numericMixing;
        const maxMixing = minMixing;

        const mixingRange = mixingNo;

        const sortedMixingNumbers = minMixing !== null ? [minMixing] : [];
        const weighted = {
        ...
        sendProgress(groupCounter, totalGroups);
        ...
        if (rows.length > 0) {
          console.log(`[Summary][Backend] Processing group mixing_no=${issue.mixing_no}, rows=${rows.length}: foundLots=${foundLots}, missingLots=${missingLots}, totalBales=${totalBales}`);
        }
        ...
        const { groupKey, versionNumber } = parseCottonVersion(issue.cotton);
