...
function AnalysisChart() {
  ...
  const fetchFilterOptions = useCallback(async (state = {}) => {
    ...
    const url = queryString
      ? `https://cotton-api-ekdn.onrender.com/api/cotton-mixing-summary/filters?${queryString}`
      : "https://cotton-api-ekdn.onrender.com/api/cotton-mixing-summary/filters";
  ...
  const refreshFilterOptions = useCallback(
    async (nextFilters) => {
      ...
      try {
        const optionResponse = await fetchFilterOptions(stateToUse);
        setOptions({
        ...
        setOptions(BASE_OPTIONS_STATE);
        setOptionsError("Failed to load filter options for the selected date range.");
        throw error;
      }
    },
  ...
  useEffect(() => {
    ...
    const fetchOptionsForDateRange = async () => {
      ...
      try {
        await refreshFilterOptions(filters);
      } finally {
        if (isMounted) {
  ...
  }, [filters, hasSelectedDateRange, refreshFilterOptions]);
  ...
  const handleApplyFilters = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://cotton-api-ekdn.onrender.com/api/cotton-mixing-summary?${params.toString()}`);
      if (!response.ok) {
